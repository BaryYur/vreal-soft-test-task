  ...
  (
    ...
    const handleSelectChange = React.useCallback(
      (item: TreeDataItem | undefined) => {
        ...
        if (item && item.id) {
          router.push(
            `/${routes.storageDashboard.index}/${routes.storageDashboard.directory}/${item.id}`,
          );
        }
      },
      [onSelectChange, router],
    );
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header>
    <AccordionPrimitive.Trigger
      ...
      {...props}
    >
      <ChevronRight className="toggle-only text-accent-foreground/50 mr-1 h-4 w-4 shrink-0 transition-transform duration-200" />
      {children}
    </AccordionPrimitive.Trigger>
  </AccordionPrimitive.Header>
));
